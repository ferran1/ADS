...
class PPSTest {
    ...
    @Test
    void calculateMostInvolvedEmployeesTest() {
        PPS pps = PPS.importFromXML("HvA2015_e5_p5.xml");
        pps.printPlanningStatistics();
        assertEquals(4, pps.calculateMostInvolvedEmployees().iterator().next().getAssignedProjects().size());
    }

    @Test
    void testCalculateManagedBudgetOverview() {
        PPS pps = PPS.importFromXML("HvA2015_e5_p5.xml");
        Predicate<Employee> filter = employee -> employee.getNumber() == 100257;
        assertEquals(27680, pps.calculateManagedBudgetOverview(filter).entrySet().iterator().next().getValue());
    }
}
