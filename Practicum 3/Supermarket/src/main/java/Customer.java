...
public class Customer implements Comparable<Customer> {
    ...
    public double calculateTotalBill() {
        ...
        for (Purchase purchase : items) {
            totalBill += purchase.getProduct().getPrice() * purchase.getAmount();
        }
    ...
    public Cashier selectCashier(List<Cashier> cashiers) {
        ...
        if (cashiers.size() == 1) {
            ...
            int minPassthroughTime = cashiers.get(0).expectedCheckOutTime(this.getNumberOfItems()) + cashiers.get(0).expectedWaitingTime(this);
            selectedCashier = cashiers.get(0);
            for (Cashier cashier : cashiers) {
                int passthroughTime = cashier.expectedWaitingTime(this) + cashier.expectedCheckOutTime(this.getNumberOfItems());
                if (passthroughTime < minPassthroughTime){
                    minPassthroughTime = passthroughTime;
    ...
    public int compareTo(Customer o) {
        if (this.getNumberOfItems() < o.getNumberOfItems()) {
            return 1;
        } else if (this.getNumberOfItems() > o.getNumberOfItems()) {
            return -1;
        }
        return 0;
    }
    ...
    public String toString() {
        ...
        for (Purchase item : items) {
            itemNames.append(item.getProduct().getDescription());
        }
        return String.format("Zip: %s,\tCashier: %s,\tItems: %s", getZipCode(), getCheckOutCashier().getName(), itemNames);
    }
